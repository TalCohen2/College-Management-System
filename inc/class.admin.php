...
class Admin {
    ...
    switch ($component) {
        ...
        case 'edit':
            ...
            if(session::logged()['role_id']=='2' && $ad['role_id'] != '3' && session::logged()['id'] != $ad['id']) {
              header("location: /talcohenproject/adminstration");
            }
            else {
              $ad += $a->getRoles();
              if($notice) {
  ...
  function login() {
    ...
    if(!session::logged()) {
      ...
      header("location: /talcohenproject/{$notice}");
    }
  }
      ...
      switch($route) {
        case 'add':
          if($value['role']=='1' || $logged['role_id']=='2' && $value['role']!='3') {
            header ("location: /talcohenproject/adminstration/addAdmin/");
          }
          else {
            $flag = TRUE;
            ...
            if($flag) {
              ...
              if($a) {
                ...
                header("location: /talcohenproject/adminstration/notice/added");
              }
              else {
                header("location: /talcohenproject/adminstration/addAdmin/1");
              }
            }
            else {
              header("location: /talcohenproject/adminstration/addAdmin/2");
            }
          }
        break;
        case 'delete':
          ...
          if($logged['id']==$path['id'] || $logged['role_id']==$path['role_id'] || $path['role_id']=='1') {
            ...
            header("location: /talcohenproject/adminstration/edit/{$value}/4");
            }
            else {
              header("location: /talcohenproject/adminstration");
            }
            ...
            header("location: /talcohenproject/adminstration/notice/deleted");
          }
        break;
        case 'edit':
          if($value['role']=='1' && $logged['role_id']!='1' || $value['role']=='1' && $logged['id']!=$value['id'] || $logged['role_id']=='2' && $value['role']!='3' && $value['id']!=$logged['id'] || $value['id']==$logged['id'] && $value['role']!=$logged['role_id']) {
            if($value['id']==$logged['id'] && $value['role']!=$logged['role_id']) {
              header("location: /talcohenproject/adminstration/edit/{$value['id']}/3");
            }
            else {
              header("location: /talcohenproject/adminstration/edit/{$value['id']}/4");
            }
          }
          else {
            $flag = TRUE;
            ...
            if($flag) {
              ...
              if($a) {
                ...
                header("location: /talcohenproject/adminstration/notice/edited");
              }
              else {
                header("location: /talcohenproject/adminstration/edit/{$value['id']}/1");
              }
            }
            else {
              header("location: /talcohenproject/adminstration/edit/{$value['id']}/2");
            }
          }
        break;
      }
